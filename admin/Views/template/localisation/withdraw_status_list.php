                   ...
                   <table id="table-location" class="table table-striped table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th width="70%"><?php echo $column_name; ?></th>
                            <th width="5%"><?php echo $column_action; ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if ($withdraw_statuses) { ?>
                            <?php foreach ($withdraw_statuses as $withdraw) { ?>
                                <tr>
                                    <th scope="row"> <?php if (in_array($withdraw['withdraw_status_id'], $selected)) { ?>
                                        <input type="checkbox" name="selected[]" value="<?php echo $withdraw['withdraw_status_id']; ?>" checked="checked"/>
                                    <?php } else { ?>
                                        <input type="checkbox" name="selected[]" value="<?php echo $withdraw['withdraw_status_id']; ?>"/>
                                        <?php } ?> </div></th>
                                        <td><?php echo $withdraw['name']; ?></td>
                                        <td class="text-center">  
                                            <a href="<?php echo $withdraw['edit']; ?>" class="btn btn-primary btn-sm" data-toggle="tooltip" data-placement="top" title="<?php echo $button_edit; ?>" ><i class="far fa-edit"></i></a></td>
                                        </tr>
                                    <?php } ?>
                                <?php } ?>
                            </tbody>
