            ...
            <div class="card-body">
               <form class="form-horizontal" enctype="multipart/form-data" id="form-location"> 
                   ...
                   <table id="table-location" class="table table-striped table-bordered">
                    ...
                    <tbody>
                        <?php if ($dispute_statuses) { ?>
                            <?php foreach ($dispute_statuses as $dispute) { ?>
                                <tr>
                                    <th scope="row"> <?php if (in_array($dispute['dispute_status_id'], $selected)) { ?>
                                        <input type="checkbox" name="selected[]" value="<?php echo $dispute['dispute_status_id']; ?>" checked="checked"/>
                                    <?php } else { ?>
                                        <input type="checkbox" name="selected[]" value="<?php echo $dispute['dispute_status_id']; ?>"/>
                                        <?php } ?> </div></th>
                                        <td><?php echo $dispute['name']; ?></td>
                                        <td class="text-center">  
                                            <a href="<?php echo $dispute['edit']; ?>" class="btn btn-primary btn-sm" data-toggle="tooltip" data-placement="top" title="<?php echo $button_edit; ?>" ><i class="far fa-edit"></i></a></td>
                                        </tr>
                                    <?php } ?>
                                <?php } ?>
                            </tbody>
