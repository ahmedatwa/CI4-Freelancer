            ...
            <div class="card-body">
               <form class="form-horizontal" enctype="multipart/form-data" id="form-location"> 
                   ...
                   <table id="table-location" class="table table-striped table-bordered">
                    ...
                    <tbody>
                        <?php if ($project_statuses) { ?>
                            <?php foreach ($project_statuses as $project_status) { ?>
                                <tr>
                                    <th scope="row"> <?php if (in_array($project_status['project_status_id'], $selected)) { ?>
                                        <input type="checkbox" name="selected[]" value="<?php echo $project_status['project_status_id']; ?>" checked="checked"/>
                                    <?php } else { ?>
                                        <input type="checkbox" name="selected[]" value="<?php echo $project_status['project_status_id']; ?>"/>
                                        <?php } ?> </div></th>
                                        <td><?php echo $project_status['name']; ?></td>
                                        <td class="text-center">  
                                            <a href="<?php echo $project_status['edit']; ?>" class="btn btn-primary btn-sm" data-toggle="tooltip" data-placement="top" title="<?php echo $button_edit; ?>" ><i class="far fa-edit"></i></a></td>
                                        </tr>
                                    <?php } ?>
                                <?php } ?>
                            </tbody>
